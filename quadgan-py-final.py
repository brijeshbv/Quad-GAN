...
def plot_results(batch, dir=None, filename="z"):
	if dir is None:
		raise Exception()
	try:
		os.mkdir(dir)
	...
	for n in range(batch.shape[0]):
		ax.scatter3D(batch[:2000,0], batch[:2000,1], batch[:2000,2], c=batch[:2000,0], marker='.')
		ax.set_xticks([-1, 0, 1])
		ax.set_yticks([-1, 0, 1])
		ax.set_zticks([-1, 0, 1])
	ax.set_xlabel('x')
...
for it in range(0,epochs):
	...
	plot_results(sample_x_real,'./quadGAN','real_samples')
	sample_XZy = FakeYRealXZ.cpu().data.numpy()
	sample_YXz = FakeZRealYX.cpu().data.numpy()
	sample_ZYx = FakeXRealZY.cpu().data.numpy()
	plot_results(sample_XZy,'./quadGAN', str(it) + 'FakeYRealXZ')
	plot_results(sample_YXz,'./quadGAN', str(it) + 'FakeZRealYX')
	plot_results(sample_ZYx,'./quadGAN', str(it) +'FakeXRealZY')
